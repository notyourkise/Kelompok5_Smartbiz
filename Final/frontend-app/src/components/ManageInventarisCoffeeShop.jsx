...
const ManageInventarisCoffeeShop = () => {
  ...
  const fetchInventaris = async () => {
    ...
    try {
      const response = await axios.get("http://localhost:3001/api/inventaris?category=coffee shop", {
  ...
  return (
    <div className="manage-inventaris-container"> 
      <header className="manage-inventaris-header">
        ...
        <h2 className="manage-inventaris-title">Manajemen Inventaris Coffee Shop</h2>
            ...
            {inventaris.length > 0 ? (
              inventaris.map((item, index) => (
                <tr key={item.id}>
                  ...
                  <td>
                    {item.stock}
                    {/* Check if stock is less than or equal to minimum_stock */}
